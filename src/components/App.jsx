...
function App() {
  const [subscribe, setSubcsribe] = useState(
    JSON.parse(localStorage.getItem('subcribe')) || false
  );
  const [followers, setFollowers] = useState(
    JSON.parse(localStorage.getItem('countFollowers')) || 100500
  );
      ...
      <ul className="section_list">
        <li className="section_item">
          ...
          <button
            ...
            className={
              subscribe
                ? 'section_button--active section_button'
                : 'section_button'
            }
            onClick={addFollower}
          >
